...
export const updateUser = async (userId, updateData) => {
    try {
        const config = getAuthHeaders();
        const { data } = await axios.put(`${API_URL}/users/${userId}`, updateData, config);
        return data;
    } catch (error) {
        throw error.response?.data?.message || error.message;
    }
...
// FR-3.5: Admin: Fetch ALL Posts (including Drafts/Suspended)
...
export const toggleCommentSuspend = async (commentId) => {
    try {
        ...
        const { data } = await axios.put(`${API_URL}/comments/${commentId}/suspend`, {}, config);
...
export const suspendPostAdmin = async (postId) => {
    try {
        ...
        const { data } = await axios.put(`${API_URL}/posts/admin/${postId}/suspend`, {}, config);  
