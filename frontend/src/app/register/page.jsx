  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      if (userData?.isVerified === false) {
        ...
        dispatch(authSuccess(userData)); // Normal login
      }
    } catch (err) {
...
=======
// blog-application/frontend/src/app/register/page.jsx
'use client';
...
const RegisterPage = () => {
    ...
    const submitHandler = async (e) => {
        ...
        try {
            ...
            // Dispatch success (logs user in immediately after registration)
            dispatch(authSuccess(userData));
            router.push(`/verify-email?email=${email}`);
        } catch (err) {
            // Error message from backend (e.g., 'User already exists')
            const errorMessage = err.response?.data?.message || err.message;
            dispatch(authFailure(errorMessage));
        }
    };
