...
import { uploadImage } from '@/utils/uploadApi'; // <-- Import the upload utility
 ...
 */
const ImageUploader = ({ initialUrl, onUrlChange }) => {
    ...
    return (
        <div className="space-y-4 p-4 border rounded-lg bg-gray-50">
            <label className="block text-sm font-medium text-gray-700">Featured Image Upload</label>
            
            {/* Display Error/Loading */}
            {error && <p className="text-sm text-red-600 font-semibold">{error}</p>}
            {loading && <p className="text-sm text-blue-600 font-semibold animate-pulse">Uploading to Cloudinary...</p>}
            ...
            <div className="flex space-x-3">
                ...
                <button
                    ...
                    className={`px-4 py-2 text-white font-medium rounded-lg transition-colors ${
                        loading || !file ? 'bg-gray-400 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700'
                    }`}
                >
